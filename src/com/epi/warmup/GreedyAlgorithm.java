...
public class GreedyAlgorithm {
	// Greedy Algorithms and Invariants: Write a program that takes an input a
	// positive integer
	// N, and returns the minimum number of coins in the US coinage system to
	// create N cents. For example, if
	// N = 37, the answer is 4, corresponding to a quarter, a dime, and two
	// pennies.
	...
	// will return the answer 3 using coins {10, 1, 1} whereas the true minimum
	// is 2 {6, 6}.

	// We require a dynamic programming algorithm for the generic case of
	// finding the minimum of coins
	// to make change for a sum j where the coin denomination is denom[1] >
	// denom[2] > ··· > denom[n] = 1.
	...
	// only coin {1}.
	// M[2][12] gives us the minimum number of coins for j=12 using coins {1, 6,
	// 10}.

	// For each coin from i to n we can choose whether to include it or not.
	// If we do not include a coin then the minimum is the minimum from the
	// previous coin.
	// minimum(j) = M[i-1][j]
	// eg not including coin i=3(10) for j=12
	// minimum(j) = M[3-1][12] = M[2][12] = 2

	// If we include a coin, then the minimum is the current coins last minimum
	// at (j-coin value)
	...
	// How do we choose whether to include a coin or not? If the coin value is
	// greater than j
	// then obviously we are not going to include it. Otherwise we must
	// calculate both and
	// use the minimum value.

	// Time complexity of this algorithm is O(MN) where M is the number of coins
	// in the denomination
	// and n is the sum j. We create a double array of coin.length and n values,
	// so the space
	// complexity is also O(MN).

	public static int minNoOfCoins_dynamic(int n, int[] coins) {

		// assert n is greater than 0, there are coins specified, and first coin
		// = 1
		assert (n > 0);
		...
		for (int j = 1; j <= n; j++) {
			...
			for (int i = 1; i < coins.length; i++) {
				if (coins[i] > j) {
					// the coin value is greater than the current sum, so don't
					// include it
					// instead we use the minimum of the previous coin
					M[i][j] = M[i - 1][j];
				} else {
					// otherwise we use either the minimum of the previous coin
					// or we use
					// add one two to the current coins previous value (j - coin
					// value)
	...
	// hence this algorithm is very slow
	...
	public static int minNoOfCoins_recursiveWithMemoization(int val,
		...
		if (M[val] > 0) {
			System.out.println("Returning cached value for " + val + " min is " + M[val]);
			return M[val];
		}
		...
		for (int i = 0; i < coins.length; i++) {
			if (coins[i] < val) {
				int coinNo = 1 + minNoOfCoins_recursiveWithMemoization(val
						- coins[i], coins, M);
				if (coinNo < min) {
					min = coinNo;
					M[val] = min;
	...
	public static void main(String args[]) {
		...
		assert (minNoOfCoins_recursiveWithMemoization(1, US_COINS, new int[1+1]) == 1);
		assert (minNoOfCoins_recursiveWithMemoization(5, US_COINS, new int[5+1]) == 1);
		assert (minNoOfCoins_recursiveWithMemoization(37, US_COINS, new int[37+1]) == 4);
